...
function updateFileList(files) {
  ...
  fileList.innerHTML = ""; // Hapus daftar file lama
  for (const file of files) {
    dataTransfer.items.add(file);

    // Menampilkan nama file tanpa preview gambar
    const li = document.createElement("li");
    li.textContent = file.name; // Hanya menampilkan nama file
    fileList.appendChild(li);
  }
  fileInput.files = dataTransfer.files; // Set input files agar sesuai
}
